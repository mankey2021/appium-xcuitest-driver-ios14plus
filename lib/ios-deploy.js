...
class IOSDeploy {
  ...
  async launchApp(app, processArguments) {
    try {
      proc = new _teen_process.SubProcess(IOS_DEPLOY, ['--id', this.udid, '--bundle', app, '--noinstall', '--debug', '--args', processArguments.toString().replace(/,/g, " ")]);
      let sd = (stdout) => {
          return stdout.indexOf("fopen failed for data file") !== -1;
      };
      await proc.start(sd);
    } catch (err1) {
